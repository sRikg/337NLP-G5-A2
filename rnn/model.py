...
class RNN(nn.Module):
    ...
    def __init__(self, num_layers, num_tokens, dim_emb, batch_size):
        ...
        self.h1 = torch.zeros(self.batch_size, 100)
        self.h2 = torch.zeros(self.batch_size, 100)
        # Tie weights
        self.init_weights()
    ...
    def forward(self, input):
        logits = torch.tensor(self.num_tokens)
        # Process a batch of 20 samples in each time step
        for col in range(input.shape[1]):
            print('embedding on CUDA?', self.embeddings.is_cuda)
            curr_input = input[:, col].long()
            print('current input on CUDA?', curr_input.is_cuda)
            x = self.embeddings(curr_input)    # should be size([20, 100])
            print('current embed on CUDA?', x.is_cuda)
            # print('shape of x:', x.shape)
            self.h1 = self.activation(self.linear1(torch.cat((x, self.h1), dim=1)))
            self.h2 = self.activation(self.linear2(torch.cat((self.h1, self.h2), dim=1)))   # size([20, 100])
